...
export function HeroListItem({
  ...
  return (
    <div
      onClick={onSelect}
      className="flex items-center gap-2.5 cursor-pointer relative py-2.5 pr-3.5 border-b border-border-subtle transition-colors"
      style={{
    ...
    >
      <div className="shrink-0 relative overflow-hidden w-8 h-8">
        <HeroPortraitArt hero={hero} variant="thumb" sizes="32px" />
      </div>
      <div className="flex-1 min-w-0">
        <div
          className="uppercase font-(--font-display) text-[15px] leading-none tracking-[0.02em]"
          style={{ color: isActive ? "var(--text-amber)" : "var(--text-hi)" }}
        >
          {hero.alias}
        </div>
        <div className="uppercase whitespace-nowrap overflow-hidden text-ellipsis text-[8px] tracking-[0.14em] text-muted-text mt-0.75">
          {(hero.labels ?? []).join("  ")}
        </div>
      </div>
      {showShiftToggle && (
        <button
          ...
          className="shrink-0 grid place-items-center w-5.5 h-5.5 text-[11px] font-bold transition-colors"
          style={{
